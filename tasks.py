...
from src.backup_cli import full_backup, differential
...
def get_current_build():
    ...
    if build_path.is_file():
        ...
        sys.exit(
            f"Die neueste Version {VERSION} wurde noch nicht gebaut. Führe erst 'invoke build' aus."
        )
...
def load_fixtures(c):
    ...
    for target in targets:
        c.run(
            f"python manage.py load_fixture --target {target['target']} --fixture_name {target['fixture_name']}"
        )
...
@task
def build(
    c, tests: bool = True, build_static: bool = True, install_requirements: bool = True
):
    """
    ...
    if Path("./build/").is_dir():
        if Path(f"./build/{PROJECT_NAME}-{VERSION}.pyz").is_file():
            response = input(
                f"Version {VERSION} existiert bereits. Trotzdem bauen und ersetzen? y/n: "
            )
            if not response == "y":
                print("breche ab")
                return
    ...
    if install_requirements:
        ...
        c.run(
            f"{BASE_DIR}/build_venv/bin/pip install -r requirements/prod.txt --target dist/ -q --no-deps"
        )
    ...
    if os.environ.get("USING_DOCKER"):
        shiv_path = "shiv"
    c.run(
        f"{shiv_path} --site-packages dist --compressed -p '/usr/bin/env python{PYTHON_VERSION}' -o build/{PROJECT_NAME}-{VERSION}.pyz -e {BUILD_ENTRYPOINT}"
    )
...
def deploy_to_target(c, target, copy_static=True):
    ...
    c.run(
        f"ssh {target} ln -sf {project_path}/build/{PROJECT_NAME}-{VERSION}.pyz {project_path}/{PROJECT_NAME}.pyz"
    )
...
def deploy_test(c, copy_static=True):
    ...
    user_input = input(
        f'Möchtest du den build "{PROJECT_NAME}-{VERSION}.pyz" auf dem Test-Server veröffentlichen? Tippe "test": '
    )
    if user_input == "test":
...
def deploy_prod(c, copy_static=True):
    ...
    """
    user_input = input(
        f'Möchtest du den build "{PROJECT_NAME}-{VERSION}.pyz" auf dem Produktiv-Server veröffentlichen? Tippe "produktiv": '
    )
...
def create_translation(c, translation="en"):
    ...
    """
    c.run(
        f"python manage.py makemessages -i venv --locale {translation} --domain django"
    )
